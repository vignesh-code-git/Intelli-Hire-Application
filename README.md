...
Each side is configured with **the other side's URL** — the frontend calls the
backend, and the backend must allow the frontend's origin. Setting either one
to its own URL breaks the pairing.
...
Both values also default to the URLs above in code, so a redeploy alone is
enough. Setting them explicitly in each dashboard is still preferred, and
required if either URL changes.

**Changing a URL means updating both sides.** A mismatch fails quietly: the API
answers `200 OK` with the full payload and the browser discards it, which looks
like a dead backend but isn't. The jobs page detects this case and names it.

> Render's free tier sleeps after ~15 minutes idle, so the first request can
> take up to a minute. The frontend shows a loading state and recovers on its
> own once the service wakes.
...
- **Environment variable:** `NEXT_PUBLIC_API_URL` = `https://intelli-hire-application.onrender.com` — the **Render** backend it calls, not this site's own address
...
| `CORS_ALLOWED_ORIGINS` | Render (backend) | **Vercel** frontend origin(s) allowed to call the API, comma-separated |
| `NEXT_PUBLIC_API_URL` | Vercel (frontend) / `frontend/.env.local` | **Render** backend base URL the app fetches from |
